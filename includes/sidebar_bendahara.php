...
<aside class="w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0">
    ...
    <div class="flex-1 overflow-y-auto py-4">
        <nav class="px-3 space-y-1">
            ...
            <a href="input_realisasi.php" class="<?php echo $current_page == 'input_realisasi.php' ? 'bg-primary text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'; ?> group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors">
                <i data-lucide="file-edit" class="<?php echo $current_page == 'input_realisasi.php' ? 'text-white' : 'text-slate-400 group-hover:text-slate-500'; ?> mr-3 flex-shrink-0 h-5 w-5"></i>
                Input Realisasi
            </a>
            <a href="daftar_realisasi.php" class="<?php echo $current_page == 'daftar_realisasi.php' ? 'bg-primary text-white' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'; ?> group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors">
                <i data-lucide="list" class="<?php echo $current_page == 'daftar_realisasi.php' ? 'text-white' : 'text-slate-400 group-hover:text-slate-500'; ?> mr-3 flex-shrink-0 h-5 w-5"></i>
                Daftar Realisasi
            </a>
        </nav>
    </div>
    ...
    <div class="p-4 border-t border-slate-200">
        <div class="flex items-center gap-3 px-3 py-2 rounded-lg bg-slate-50">
            <div class="h-8 w-8 rounded-full bg-navy-800 text-white flex items-center justify-center font-bold text-xs">
                <?php echo strtoupper(substr($_SESSION['nama'] ?? 'B', 0, 1)); ?>
            </div>
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-slate-900 truncate"><?php echo htmlspecialchars($_SESSION['nama'] ?? 'Bendahara'); ?></p>
                <p class="text-xs text-slate-500 truncate">Bendahara Desa</p>
            </div>
        </div>
